...
CONFIG_PATH = SCRIPT_DIR / "technipen_config.json"
DEFAULT_CONFIG = {
...
def load_runtime_config():
    try:
        ...
        print(f"[config] Failed to read {CONFIG_PATH.name}: {exc}", flush=True)
        loaded = {}
...
BLE_MAC = str(RUNTIME_CONFIG.get("ble_mac", DEFAULT_CONFIG["ble_mac"]))
DEVICE_NAME = str(RUNTIME_CONFIG.get("device_name", DEFAULT_CONFIG["device_name"]))
IP_ADDRESS = str(RUNTIME_CONFIG.get("ip_address", DEFAULT_CONFIG["ip_address"]))
...
VOICE_BLOCKSIZE = 0
VOICE_MIC_INDEX = int(RUNTIME_CONFIG.get("voice_mic_index", DEFAULT_CONFIG["voice_mic_index"]))
VOICE_COMMAND_TIMEOUT = 5
VOICE_AUDIO_QUEUE_MAXSIZE = 32
VOICE_MAX_BACKLOG_CHUNKS = 6
...
def start_voice_thread(voice_state, command_queue):
    ...
    def worker():
        audio_queue = queue.Queue(maxsize=VOICE_AUDIO_QUEUE_MAXSIZE)

        def callback(indata, frames, time_info, status):
            if status:
                voice_state["error"] = f"Audio status: {status}"
                if getattr(status, "input_overflow", False):
                    overflow_count = voice_state.get("overflow_count", 0) + 1
                    voice_state["overflow_count"] = overflow_count
                    voice_state["status"] = f"Mic overflow x{overflow_count}"
        ...
        try:
            ...
            try:
                ...
                input_samplerate = int(round(mic_info.get("default_samplerate") or 48000))
            except Exception as exc:
                voice_state["mic_name"] = f"Mic {VOICE_MIC_INDEX}"
                voice_state["error"] = f"Mic lookup failed: {exc}"
                input_samplerate = 48000

            voice_state["mic_samplerate"] = input_samplerate
            resample_state = None
            ...
            with sd.RawInputStream(
                ...
                while not voice_state["stop"]:
                    ...
                    chunk_frames = max(1, len(data) // 2)
                    chunk_duration = chunk_frames / max(1, input_samplerate)
                    queue_backlog = audio_queue.qsize()
                    target_backlog = 1
                    if chunk_duration > 0 and processing_time_ema > chunk_duration:
                        target_backlog = min(
                            VOICE_MAX_BACKLOG_CHUNKS,
                            max(1, int(math.ceil(processing_time_ema / chunk_duration)))
                        )

                    dropped_chunks = 0
                    while audio_queue.qsize() > target_backlog:
                        try:
                            audio_queue.get_nowait()
                            dropped_chunks += 1
                        except queue.Empty:
                    ...
                    if dropped_chunks:
                        total_dropped = voice_state.get("dropped_audio_chunks", 0) + dropped_chunks
                        voice_state["dropped_audio_chunks"] = total_dropped
                        voice_state["status"] = (
                            f"Audio catching up ({total_dropped} chunks dropped)"
                        )

                    processing_started_at = time.time()
                    if input_samplerate != VOICE_SAMPLERATE:
                    ...
                    processing_elapsed = time.time() - processing_started_at
                    if processing_time_ema <= 0:
                        processing_time_ema = processing_elapsed
                    else:
                        processing_time_ema = (processing_time_ema * 0.8) + (processing_elapsed * 0.2)
                    voice_state["audio_processing_ms"] = round(processing_time_ema * 1000, 1)
                    voice_state["audio_queue_backlog"] = queue_backlog
                    voice_state["audio_chunk_ms"] = round(chunk_duration * 1000, 1)
        except Exception as exc:
...
def connect_ble_factory(ble_status_message, ble_data):
    async def connect_ble_device():
        ...
        devices = await asyncio.wait_for(
            BleakScanner.discover(timeout=BLE_SCAN_TIMEOUT),
            timeout=BLE_SCAN_TIMEOUT + 1.0
        )
        for device in devices:
            debug_log(
                f"[ble] Discovered device name={device.name!r} address={device.address}"
            )
            if device.name == DEVICE_NAME:
                ...
                await asyncio.wait_for(client.connect(), timeout=BLE_CONNECT_TIMEOUT)
                if client.is_connected:
                    debug_log("[ble] Connected using scanned device")
                    return client

        if mac_failure is not None:
            raise RuntimeError(
                f"Device not found after MAC failure: {mac_failure}"
            )
    ...
    async def disconnect_client(client):
        ...
        try:
            if client.is_connected:
                await client.disconnect()
                debug_log("[ble] Disconnected client")
        except Exception as exc:
            debug_log(f"[ble] Disconnect failed: {exc}")

    async def read_gatt_text(client, char_uuid, axis_label):
        payload = await client.read_gatt_char(char_uuid)
        try:
            return float(payload.decode().strip())
        except Exception as exc:
            raise RuntimeError(
                f"Invalid {axis_label} payload {payload!r}: {exc}"
            ) from exc
    ...
    async def ble_loop():
        while not ble_data["stop"]:
            ...
            try:
                ...
                debug_log(f"[ble] Connection state after connect: {client.is_connected}")
                ...
                while not ble_data["stop"]:
                    ...
                    x_value = await read_gatt_text(client, CHAR_UUID_X, "X")
                    y_value = await read_gatt_text(client, CHAR_UUID_Y, "Y")
                    z_value = await read_gatt_text(client, CHAR_UUID_Z, "Z")
                    now = time.time()
                ...
                should_retry = not ble_data["stop"]
            if should_retry:
...
def debug_window_main(state_path, control_path):
    ...
    while running:
        ...
        content_left = panel_rect.left + 15
        content_top = panel_rect.top + 14
        column_gap = 22
        left_col_width = max(250, int(panel_rect.width * 0.52))
        right_col_x = content_left + left_col_width + column_gap
        right_col_width = max(180, panel_rect.right - 15 - right_col_x)
        section_gap = 14

        ble_box_height = 215
        ble_box = pygame.Rect(content_left, content_top + 38, left_col_width, ble_box_height)
        voice_box = pygame.Rect(
            content_left,
            ble_box.bottom + section_gap,
            left_col_width,
            panel_rect.bottom - 15 - (ble_box.bottom + section_gap)
        )
        controls_box = pygame.Rect(right_col_x, content_top + 38, right_col_width, panel_rect.height - 53)
        button_width = controls_box.width - 32
        toggle_rect = pygame.Rect(controls_box.x + 16, controls_box.y + 52, button_width, 36)
        cancel_voice_rect = pygame.Rect(controls_box.x + 16, toggle_rect.bottom + 14, button_width, 36)
        slider_label_y = cancel_voice_rect.bottom + 28
        ...
        for rect in (ble_box, voice_box, controls_box):
            pygame.draw.rect(screen, (236, 241, 246), rect, border_radius=12)
            pygame.draw.rect(screen, (150, 162, 176), rect, 1, border_radius=12)

        title_items = [
            ("TechniPen Debug", title_font, content_left, content_top),
            ("BLE Metrics", font, ble_box.x + 16, ble_box.y + 14),
            ("Voice Status", font, voice_box.x + 16, voice_box.y + 14),
            ("Controls", font, controls_box.x + 16, controls_box.y + 14),
        ]
        for text, used_font, x_pos, y_pos in title_items:
            screen.blit(used_font.render(text, True, (15, 20, 28)), (x_pos, y_pos))

        ble_lines = [
            f"BLE: {state.get('ble_status_message', 'Waiting...')}",
            ...
            f"Net Accel: {state.get('net_acceleration', 0.0):.6f}",
            f"Smoothing Alpha: {state.get('dynamic_follow_alpha', 0.0):.4f}",
            f"dt: {state.get('sample_dt', 0.0):.4f}s",
        ]
        ble_y = ble_box.y + 48
        for line in ble_lines:
            screen.blit(font.render(line, True, (15, 20, 28)), (ble_box.x + 16, ble_y))
            ble_y += 25

        voice_lines = [
            f"Voice: {state.get('voice_status', 'Waiting...')}",
            f"Mic: {state.get('voice_mic_index', '-')}  {state.get('voice_mic_name', '-')}",
            f"Wake: {state.get('voice_last_wakeword', '-')}",
            f"Partial: {state.get('voice_partial', '-')}",
            f"Cmd: {state.get('voice_last_command', '-')}",
            f"Intent: {state.get('voice_last_intent', '-')}",
            f"Error: {state.get('voice_error', '-')}",
            f"Status: {state.get('status_message', 'Ready')}",
        ]
        voice_y = voice_box.y + 48
        for line in voice_lines:
            screen.blit(small_font.render(line, True, (15, 20, 28)), (voice_box.x + 16, voice_y))
            voice_y += 25
        ...
        screen.blit(slider_label, (controls_box.x + 16, slider_label_y))
