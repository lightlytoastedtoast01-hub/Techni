...
DEBUG_WINDOW_SIZE = (520, 380)
HEADER_HEIGHT = 110
MIN_DRAW_WINDOW_SIZE = (700, 420)
MIN_DEBUG_WINDOW_SIZE = (420, 320)
...
def deserialize_action_list(entries):
    ...
    for entry in entries:
        ...
        if points:
            actions.append((points, color, max(1, min(40, brush_size))))
    return actions
...
def start_ble_thread(ble_status_message, ble_data):
    if BleakClient is None or BleakScanner is None:
        ble_status_message["value"] = "BLE unavailable"
        return
...
def debug_window_main(state_path, control_path):
    ...
    while running:
        ...
        texts = [
            ...
            (f"Status: {state.get('status_message', 'Ready')}", small_font, 30, 330),
        ]
...
def main():
    ...
    ble_status_message = {"value": "BLE starting..." if BleakClient and BleakScanner else "BLE unavailable"}
    ble_data = {
        "x": 0.0,
        "y": 0.0,
        "z": 0.0,
